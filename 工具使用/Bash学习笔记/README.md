...
1. shebang行
    ...
    2. 若Bash解释器不放在目录`/bin`，则脚本无法执行
        ...
        >`env`：扫描当前用户的`$PATH`环境变量；`#!/usr/bin/env 命令`：在当前用户的`$PATH`环境变量中动态查找第一个匹配的`命令`作为解释器。
...
1. 概念
    ...
    在类 Unix 系统中，进程打开的每个输入/输出对象（普通文件、目录、管道、终端、套接字、设备等），都会对应一个非负整数编号，这个编号就是**文件描述符**（file descriptor，FD）。
2. 标准文件描述符
    ...
    默认情况下，三者都连接到当前终端；其中 `stderr` 与 `stdout` 分离，便于单独处理错误信息。
3. 常见重定向

    1. `<文件` 或 `0<文件`：让 FD `0` 从「文件」读取。
    2. `>文件` 或 `1>文件`：让 FD `1` 写入「文件」，若文件已存在则先截断（文件还在，但长度被重置，通常为0，内容被清空）。
    3. `>>文件` 或 `1>>文件`：让 FD `1` 追加写入「文件」。
    4. `2>文件`：让 FD `2` 写入「文件」。

    - `n>文件`、`n>>文件`、`n<文件`：显式操作编号为 `n` 的 FD。
4. FD 复制与关闭

    1. `n>&m`：让 FD `n` 指向与 FD `m` 相同的目标。
    2. `n<&m`：让 FD `n` 指向与 FD `m` 相同的输入源。
    3. `n>&-`、`n<&-`：关闭 FD `n`。

    - `2>&1` 的含义是：让 `stderr` 指向当前 `stdout` 指向的位置。

        1. `cmd >log 2>&1`：表示先把 `stdout` 重定向到 `log`，再让 `stderr` 跟随 `stdout`，最终两者都写入 `log`。
        2. `cmd 2>&1 >log`：表示先让 `stderr` 跟随当时的 `stdout`（通常还是终端），之后才把 `stdout` 改到 `log`，所以错误输出通常仍会显示在终端。
5. Bash 常见写法

    1. `&>文件` 或 `>&文件`：把 `stdout` 与 `stderr` 一并重定向到「文件」。
    2. `<>文件`：以可读可写方式打开「文件」。
    3. `exec 3<>文件`：把「文件」打开到 FD `3`，之后可用 `read -u 3` 读取，或用 `echo ... >&3` 写入。
    4. `exec` 配合重定向时，修改的是当前 Shell 自身的 FD，而不是仅对某个子命令生效。
6. 丢弃输出

    1. `/dev/null` 可以理解为一个“黑洞文件”，写进去的内容会直接消失。
    2. `cmd >/dev/null 2>&1`

        1. `>/dev/null`：先把标准输出 `1` 丢到 `/dev/null`
        2. `2>&1`：再把标准错误 `2` 指向标准输出 `1` 当前指向的位置

        因为这时 `1` 已经指向 `/dev/null`，所以 `2` 也会一起被丢弃。

        ```shell
        ls not-exist >/dev/null 2>&1
        ```

        上面这条命令即使报错，终端也不会显示任何信息。
7. 查看与限制
