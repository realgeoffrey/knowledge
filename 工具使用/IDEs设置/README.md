...
| --------- | ----- | ------- | ------ |
| AGENTS.md | 是（隐式加载） | 否 | 描述整个项目大纲，作为AI的README，`.cursor/rules`的简化替代 |
| Rules     | 是（隐式加载） | 是（可`@`引用） | 具体规则，分割成多个rules |
| Skills    | 是（隐式加载） | 是（可`/`引用） | 流程封装、标准化（描述、指令、工具定义）<br>规定审查标准和输出格式，按需加载资源成为上下文 |
| Commands  | 否 | 是（`/`触发） | 手动触发入口，封装执行其他功能 |
| Hooks     | 是（事件触发） | 否 | 生命周期自动化：在特定事件时执行操作 |
| Subagents | 否（由AI委派） | 否 | 专项分工：上下文隔离、并行/专业化任务 |
| MCP       | 否（由AI调用） | 否 | 外部工具协议（使用MCP沟通外部服务，扩展Agent能力边界） |
