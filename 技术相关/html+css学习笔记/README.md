...
### 切图
- 语义化：不能全用div。先用纯html标签语义化结构，再加入css满足样式，最后加入交互
- 层级尽量简化：用父的class去管理子元素
- 行内元素不要嵌套块级元素（如a标签不嵌套div）
- 因为移动端大部分是webkit内核浏览器，因此可以使用较新的技术，如css3；pc端要适配到ie6，因此要渐进增强，如派生兄弟选择器不能在ie6中起作用