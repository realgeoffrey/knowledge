...
1. 媒体查询设置html的font-size，把要做成响应式的内容转换为rem单位。
    ...
    2. 需要使用css预处理语言的最优解：
        ...
        1. 正常完成切图：可以用大于320px设计稿切完图，所有单位的值用预处理语言设置为设计稿的大小传入参数。
        2. 媒体查询仅设置html的不同情况下的font-size值。
        3. 把以上使用的预处理语言设置方法为：返回参数除以正常情况的font-size再除以2（设计稿放大倍数）。如果设计稿大于320px并且想要最低适配至320px屏幕，这里返回的参数要再缩小一些。
        4. 用预处理语言生成为css文件，其中所有单位都为rem（除了部分不想改变的使用px外）
...
- `inline`节点（如img）标签前后的空格可能导致其父级的宽度变大，通过以下办法解决：
