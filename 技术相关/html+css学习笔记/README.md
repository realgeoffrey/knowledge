...
### 横竖屏切换
>- 翻转效果的节点内容，如果要增加html滚动条，不能在此标签上增加`border-radius`值，否者横竖滚动条效果相反。
>- 部分Android系统对html内嵌的滚动条（`overflow: hidden/auto`）支持不佳，尤其增加了翻转效果后，设置的滚动条会导致更多样式问题。建议用按钮（翻页或js滚动）代替html滚动条，或用`touchmove`实现滑动页面。
...
- css分类命名规范
    ...
    5. 样式区分`.i-`（icon）
        ...
        >如果在模块上，可以使用**选择器扩展**而不加前缀`.i-`，比如`.m-xxx`扩展内容：`.m-xxx-1`或`.m-xxx .btn`扩展内容：`.m-xxx .btn-1`。
    6. js功能`.j-`（javascript）
