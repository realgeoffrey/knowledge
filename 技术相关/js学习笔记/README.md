...
### prototype
1. prototype属性是js函数的继承机制，是构造函数的属性，作用是为实例共享属性（或方法），`构造函数.prototype`和`实例对象.__proto__`（已弃用）指向同一个原型链。

2. 由同一个构造函数实例化的不同对象，各自有不共享的属性和方法副本，改变一个实例内容不会影响其他实例；但共同引用的prototype对象则共享其中内容，修改了prototype对象就会影响所有该构造函数的实例。
...
3. `var a = a || {};`
    ...
    右侧的表达式`a || {}`先执行：根据规则先判断a的值是否为真（也就是判断a本身是否存在），如果a本身存在，则返回a；如果a不存在，则返回后面的{}。
    最后再将值赋值给a。
...
6. `eval`中直接执行`function`声明无效，必须用引号把`function`声明包裹起来才有效（尽量不用`eval`）
    ...
    >`if()`中的代码对于`function`的声明就是用`eval`带入方法当做参数，因此虽然返回true，但是方法没有被声明。
    >
    >`setTimeout`与`setInterval`中第一个参数若使用字符串，也是使用`eval`把字符串转化为代码。
7. 实例化（new）一个构造函数
...
11. `if`、`for`以及`while`之类的判断语句中用赋值操作

    （大部分是误用）赋值的内容Boolen后为假会导致条件判断为假：`if(a = false){/* 不执行*/}`。

    >判断语句内只判断整体返回值是true或false，与里面执行内容无关（尽管对其语法有所限制）。
12. 自动插入分号机制（Automatic Semicolon Insertion，ASI）

    ASI机制不是说在解析过程中解析器自动把分号添加到代码中，而是说解析器除了分号还会以换行为基础按一定的规则作为断句（EOC）的依据，从而保证解析的正确性。
...
8. 注释规范
    ...
    3. 函数注释规范
        ...
        /*
         * 函数说明
         * @param {Number} param1 [1|2|3] 参数描述
...
4. 用`tab`模拟点击事件：[链接](../js方法积累/实用方法#原生js移动端模拟点击事件避免300毫秒click)。
...
1. web storage（localStorage、sessionStorage）
    ...
    5. localStorage、sessionStorage区别
        ...
        1. `localStorage`
            ...
            1. 同源共享。
            2. 持久化本地存储。除非被清除，否则永久保存（因为修改和访问的都是本地文件，因此在一个会话中修改值，同域名下的其他会话的值也变化)。
            3. 5m。
            4. 应用场景：所有需要长期本地存储的数据。
        2. `sessionStorage`

            1. 同源且同会话（tab窗口）下共享。
            2. 会话级别存储。跳转页面为同源后仍旧有效，关闭浏览器后被清除（一个会话存储一个sessionStorage对象，不同tab的值不共通;关闭tab后恢复此tab可能恢复数据）。
            3. 5m。
            4. 应用场景：需要拆分成多个子页面的填写数据。
2. `cookie`：

    1. 本地保存，字符串形式保存。
    2. 若不设置失效期则存储在内存中；若设置了失效期则存储在硬盘。
    3. 同源同路径，默认是关闭浏览器后失效，设置失效时间则到期后才失效。
    4. 始终在http请求中携带。
    5. 单个cookie保存的数据不超过4k，单个域名有限制数量的cookie（最少20个）。
    6. 需要程序员自己封装get和set方法，源生的Cookie接口不友好。
    7. 应用场景：主要判断用户登录，用于辨认不同用户以提供不同个性化服务。
3. `session`：

    1. 服务端保存，对象形式保存。
    2. 无状态值（无法区分请求地址），需要借助本地cookie进行操作。
...
1. 每个函数都是一个Function对象，像普通对象一样拥有**属性**和**方法**。
    ...
    4. 函数内的`arguments.callee`是一个指针：其指向拥有`arguments`对象的函数。
