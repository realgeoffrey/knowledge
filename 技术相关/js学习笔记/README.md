    ...
    - 使用策略
        ...
        - 客户端内嵌页面
            ...
            1. 处理客户端回调函数错误
                ...
                1. 因为客户端调用前端的方法是直接通过函数运行js代码，抛出错误时`window.onerror`传入的参数仅有第一个`message`参数（`file`、`line`以及其他参数都没有），所以必须在给客户端调用的js方法中嵌套`try-catch`并且抛出能标识出所调用方法名字的错误信息。
                2. 又因为要避免**js代码还未加载完毕客户端就调用回调函数**的情况，要让客户端调用回调函数的时候嵌套一层`try-catch`，在`catch`中提供调用哪个方法（和使用哪个文件路径）的信息。

            2. 同样也需要在加载js之前配置好`window.onerror`，来处理页面内错误。
