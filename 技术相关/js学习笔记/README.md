...
- `var a = [10, 20, 30, 40][1, 2, 3];   /* 40*/`
    ...
    - `[1, 2, 3]`被解析为属性调用，逗号操作符取最后一个值为结果。

    因此结果为数组`[10, 20, 30, 40]`的`[3]`属性值：`40`。
- html5的`audio`标签的自动播放属性`autoplay`

    ios系统是无法自动播放，可以设置触屏的时候开始播放：
    ...
    ```
- `eval`中直接执行`function`声明无效，必须用引号把`function`声明包裹起来才有效（尽量不用`eval`）
...
- 浮点数的计算
    ...
    >避免浮点数运算误差算法:[链接](https://github.com/realgeoffrey/knowledge/blob/master/技术相关/js方法积累/实用方法/README.md#原生js用整数进行小数的四则运算避免浮点数运算误差)。
- 判断DOM是否支持某属性
...
- 当比较的两个值的类型不同时，`==`和`!=`都会强制类型转换，再进行转换后值的比较；用`===`和`!==`则不会转换，直接返回`false`（`switch`语句比较值是全等模式比较）。
...
- 注释规范
    ...
    - 函数注释规范
        ...
        ```
- js编程风格总结<[链接](http://www.ruanyifeng.com/blog/2012/04/javascript_programming_style.html)>
