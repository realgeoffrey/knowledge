...
- 元字符
    ...
    - 字符集合
        ...
        | 字符 | 表示的意义 |  | 字符 | 表示的意义 |
        | :---: | :--- | --- | :---: | :--- |
        | `[xyz]` | 一个字符集合，也叫字符组。匹配集合中的任意一个字符。也以使用连字符`-`指定一个范围。 |  | |  |
        | `[^xyz]` | 一个反义或补充字符集，也叫反义字符组。也就是说，它匹配任意不在括号内的字符。也可通过使用连字符`-`指定一个范围内的字符。 |  |  |  |
