...
>2. 偏移量：一个数字（或字符串），表示特定日期/时间值比UTC早或晚多少，e.g. '+0800'、'GMT-0800'。
></details>
...
2. 在A时区存储时间，在A或其他时区展示，期望展示同一个时间刻度：
    ...
    1. 方案一：不包含时区、不包含偏移量的时间字符串（不能也不需要进行偏移，表示某个固定时区或固定偏移（如：UTC+0或服务端时区）的时间刻度）
...
- 各种相关时间表达方式能够获取的信息
    ...
    3. 时间戳：若确定时区或偏移量后，则可以获得时间刻度（因此不同时区或不同偏移量会得出不同时间刻度，默认可以取本地的时区）
    4. 时间戳+时区（`IANA时区标识符，如"America/New_York"`）：（地区+）时区+时间刻度+偏移量（因为有夏令时/冬令时的原因，所以时区算出的偏移量是会根据时间戳不同而变化）
        ...
        moment.tz(1741505400000,'America/New_York').format() === moment(1741505400000).tz('America/New_York').format()
        ```
        </details>
    5. （包含偏移量的时间字符串）`2012-11-04T01:00:00-04:00`+时区：包含偏移量的时间字符串 等价于 时间戳，因此等于上一条
        ...
        moment.tz('2025-03-09T02:30:00-05:00','America/New_York').format() === moment('2025-03-09T02:30:00-05:00').tz('America/New_York').format()
        ```
        </details>
    6. （不包含偏移量的时间字符串）`2012-11-04T01:00:00`+时区：若表达的是时区下的时间刻度，则可以算出时间戳，那么就等价于上上一条

        要注意实现逻辑，优先使用`moment.tz(不包含偏移量的时间字符串, 时区)`，而不是 ~~`moment(不包含偏移量的时间字符串).tz(时区, true)`~~。
