...
- 一行文字中间需要插入HTML标签的情况（简单的数量或变量替换直接用占位符即可，不涉及插入html、XSS）
    ...
    >2. 插入html（`innerHTML`）考虑XSS，应尽量避免
    ...
    1. （直接翻译）`$t('将文件拖到此处，或<em>点击上传</em>')`
        ...
        XSS风险高。若是机翻，则可能对html标签处理不好；若是经过培训的人工翻译，能够解决翻译内容中包含html标签的情况
    1. （占位符）`$t('将文件拖到此处，或{0}点击上传{1}').解决xss().replace('{0}', '<em>').replace('{1}', '</em>')`

        无XSS风险。占位符作为html标签语义不明
    2. `$t('将文件拖到此处，或${0}').replace('${0}', "<em>{{ $t('点击上传') }}</em>")`

        XSS风险高。语序被分割，翻得不自然。不适用于语序变化强烈的语言
    3. `$t('将文件拖到此处，或{0}点击上传').split('{0}')[0]` + `<em>` +`$t('将文件拖到此处，或{0}点击上传').split('{0}')[1]` + `</em>`

        无XSS风险。代码可读性差，占位符作为分隔符语义不明
