...
2. 分页加载、滚动加载
    ...
    2. 滚动加载：关键是「下一批从哪接着取」，一般用 **游标**（或等价的位置标记）。
        ...
        - 游标由谁维护
            ...
            1. 常规列表：前端（或客户端） 保存服务端回传的游标；下次请求带上 `游标 id`，响应再给出 下一枚游标 id。
            2. 时效强或强个性化场景（如推荐流、已推送不再出现、点「不感兴趣」后过滤等）：服务端 用 Redis 等按用户或 session 记录进度；前端可反复请求同一接口（或仅带固定参数），分页由服务端实现。

    >- 若用分页加载的服务端接口实现滚动加载
    >
    >    1. 则可能出现请求到重复数据（前面插入数据）或略过数据（前面删除数据）的情况。（游标的，若没有管理好数据流，则也会出现重复数据或略过数据情况）
    >    2. <details>
    >
    >        <summary>前端（或客户端）也可以模拟游标管理方式：暴露一个加载更多的无参数接口，在接口内部实现类似服务端的游标管理</summary>
    >
    >        ```js
    >        let arr = []    // 数据
    >        const size = 10 // 每页数量
...
7. 若`遇到较复杂/交互冗长的需求` && `后端开发没有比你更了解需求/没有给出能够完整链路联调的数据`时，则在充分理解需求后，由前端自行mock请求数据：

    0. 分阶段、逐步澄清原始诉求和需求意图
    1. 与后端确认主流程所需的接口数量，并初步约定参数

        1. 接口契约：`method`、`path`、`query`、`request body`
        1. 返回契约：`response body`
    2. 前端请求层实现mock时的注意事项

        1. 统一配置

            1. 所有mock相关配置统一放在mock配置文件中，可支持用`环境变量、URL参数`控制；禁止把mock开关散落在页面组件里

                如：mock总开关、请求耗时、请求错误概率、慢请求概率、超时概率
            1. 请求耗时：模拟正常耗时、慢请求、超时

                平时开发可设置固定耗时（如：500ms），再配置小概率触发的慢请求或超时。
            1. 请求错误

                通过开关、错误概率控制。统一配置只处理http请求错误；业务失败放到每个接口mock中实现。
            1. 统一入口：页面仍调用真实请求方法；mock只替换网络层或service实现，避免切换真实接口时大改页面逻辑
        1. 每个业务请求中实现（模拟真实环境）

            1. 业务逻辑mock放到对应请求方法中
            1. mock方法要打印出入参详细与出参详细

                1. 入参详细：与真实请求一致的完整样例（`path` 参数、`query`、`request body`）
                1. 出参详细：完整 `response body`（含业务成功/失败及关键字段）
            1. 数据规模：（分页/滚动）覆盖 空列表、1条、刚好一页、超过一页，第一页、中间页、最后一页、无下一页
            1. 字段边界，如：空值、长文本、特殊字符、XSS字符串、中文/英文混排、超大数字、小数精度、负数、0、时间跨天/跨月/跨年/时区
            1. 业务状态：覆盖所有枚举值、状态流转前/中/后、不可操作状态/空状态
            1. （特殊业务）不可使用的字段
