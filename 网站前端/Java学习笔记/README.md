...
- 多线程基础：先掌握 `Thread`、`Runnable`、线程池、锁、可见性、原子性，再深入并发工具类。进程内定时见 `ScheduledExecutorService`；业务 cron / 集群调度见后文[定时任务](#定时任务)。
...
- **Maven**
    ...
    1. 配置入口
        ...
        1. 项目构建模型：[`pom.xml`](./pom.xml)
            ...
            1. Maven 默认目录约定
                ...
                >资源由资源插件复制到输出目录，不经 Java 编译器编译。Classpath（类路径）是查找类和资源的目录 / jar 路径，Maven 按 scope 组织编译、测试和运行的 classpath。
    ...
    1. <details>
        ...
        1. 仓库类型
            ...
            1. **远程仓库**：通过 URL 访问，常用 `https`，也支持 `file` 等。
            1. **中央仓库**：默认公共仓库 `central`，地址 `https://repo.maven.apache.org/maven2`。
            1. **第三方仓库**：厂商或组织提供的额外制品来源。
            1. **私服 / 内部仓库**：如 Nexus、Artifactory，用于代理上游、托管内部制品和统一权限。
            1. **依赖仓库**：`repositories` 配置项目依赖下载源。
            1. **插件仓库**：`pluginRepositories` 配置插件及其依赖下载源。
            1. **发布仓库**：`distributionManagement` 配置 `deploy` 上传目标。
            1. **镜像仓库**：settings 的 `mirrors` 替换匹配的下载源，不改变发布目标。
        ...
        1. 跨项目按坐标解析制品：独立构建的本地项目联调，先 `install` 上游，仅 `package` 不会更新本地仓库；同一次 reactor 构建可直接使用模块结果。

        </details>
    1. 构建执行、生命周期与问题排查

        1. 生命周期分三条独立线：`clean`、`default`（日常构建）、`site`（站点文档）。默认生命周期常用阶段为 `validate → compile → test → package → verify → install → deploy`（中间还有若干阶段，日常可省略记忆）；执行某一阶段会先执行**同一条生命周期**上的前置阶段，因此 `package` 不会顺带执行 `clean`。
        ...
        1. 常见构建配置：编译目标、编码、资源、测试、打包及质量检查。

            - `mvn -v` 查看运行 Maven 的 JDK。JDK 9+ 编译优先用 `release` 约束语言、字节码和 Java SE API；`source` / `target` 不能阻止误用新 API。
            ...
            - 普通 jar 默认不包含测试代码，也不把依赖 jar 打进去；Spring Boot 可执行包需启用 `spring-boot-maven-plugin` 的 `repackage`（把依赖重打包进 fat jar）。
        ...
        1. 多模块一次构建时，Maven 用 reactor 按模块间依赖等关系排序：`-pl` 选模块（相对路径或 `:artifactId`），`-am` 追加所选模块需要的上游模块，`-amd` 追加依赖所选模块的下游模块。仅出现在 `dependencyManagement` / `pluginManagement` 中、未形成真实模块依赖的，不参与排序关系。
        1. 阶段效果与边界

            | 阶段 | 作用 |
            | --- | --- |
            | `clean` | 清理构建输出（通常为 `target`），不清空本地仓库 |
            | `test` | 通常由 Surefire 运行单元测试 |
            | `package` | 按 packaging 生成制品 |
            | `verify` | 执行已绑定的校验，不自动配置集成测试 |
            | `install` | 将 POM、主制品和附加制品安装到本地仓库 |
            | `deploy` | 发布制品到远程仓库，不是启动应用 |

            单元测试通常由 Surefire 跑在 `test` 阶段；集成测试通常由 Failsafe 绑定到 `integration-test` 与 `verify`。跑集成测试用 `mvn verify`，以便执行测试后的清理与结果检查；只跑到 `package` / `integration-test` 可能漏掉这些步骤。

        1. 跳过测试：`-DskipTests` 跳过 Surefire 执行但仍编译测试代码；`-DskipITs` 跳过 Failsafe 集成测试；`-Dmaven.test.skip=true` 同时跳过测试编译和执行。**Failsafe 3.6.0 起不直接响应 `skipTests`**（需用 `skipITs` 等），旧版或自定义绑定另核。日常校验不要默认跳过测试。
        ...
        1. 问题排查
            ...
            | 问题 | 优先检查 |
            | --- | --- |
            | 配置生效情况 | `mvn help:effective-pom -Dverbose` 看 POM 及来源；`mvn help:effective-settings` 看 settings；`mvn help:active-profiles` 看 profile |
            | 依赖冲突 | `mvn dependency:tree`，可加 `-Dincludes=groupId:artifactId`；`NoSuchMethodError` 还需核对部署包 / 运行 classpath |
            | 下载失败 | 查坐标、仓库 / 镜像、server id、网络和版本策略；修正后可用 `-U` 重试缓存的失败请求 |
            | Java 版本不符 | 对照 Maven / IDE / 应用 JDK、Toolchains 与 `release` |
            | 测试异常 | 查测试插件、发现规则、跳过参数及 `target/surefire-reports` / `target/failsafe-reports` |

            先定位失败模块、phase、goal 和根本异常，按需加 `-e` / `-X`，避免直接清空本地仓库。
...
2. JDBC 实际使用重点
    ...
    | 事务 | 同一事务的所有 SQL 必须共用同一个 `Connection`；关闭自动提交后，成功调用 `commit()`，失败调用 `rollback()` |
    ...
    - <details>
        ...
        - **数据库连接池**：是池化技术的一种应用。建立数据库连接需要网络通信、身份验证和会话初始化；连接池在应用内管理一组可复用的连接，避免每次执行 SQL 都重新建立物理连接。
            ...
            | 常见配置 | 作用 |
            | --- | --- |
            | 最大连接数 | 限制应用同时占用的数据库连接；不是越大越好 |
            | 最小空闲连接数 | 控制预留多少可立即使用的连接 |
            | 获取连接超时 | 连接池已满时，请求最多等待多久 |
            | 空闲超时与最大存活时间 | 回收长期闲置或过旧的连接，避免使用已被数据库或网络断开的连接 |

            - 连接池耗尽时，新请求会等待并最终超时。常见原因是连接未归还、慢 SQL、长事务，或连接池容量与并发量不匹配。
            - 容量规划还要考虑数据库侧的总连接上限。
        </details>
    ...
    - <details>
        ...
        - Spring 项目通常在 Service 层使用 `@Transactional`，由框架管理提交和回滚；具体规则见后文 Spring Framework 事务小节。
        </details>
...
1. Spring Boot：整合与快速启动
    ...
    1. **数据访问、事务与外部服务整合**
        ...
        - 扩展能力按业务需要补：Redis 缓存、消息队列、异步任务、[定时任务](#定时任务)、邮件、对象存储、第三方 HTTP 客户端。
...
1. 怎么选（懒人规则）。
    ...
    ><summary>与前端 / Node.js 类比</summary>
    >
    >1. `Timer` / `ScheduledExecutorService` 类似 `setTimeout` / `setInterval`，但跑在服务端进程里，进程一挂就停。
    >1. Spring `@Scheduled` 类似进程内 `node-cron`，配置写在应用里。
    >1. XXL-JOB（或公司内部 Chronus）类似独立的定时控制台，cron 和执行日志不跟代码绑死。
    >1. MQ 延时类似队列里的 delayed job（Bull / Agenda 的 delay），不是日历闹钟。
    ></details>
...
- `DataSource` / 连接池 / 池化：`DataSource` 是取数据库连接的标准入口。生产环境用连接池（如 HikariCP）：预先准备若干连接，用时借出、用完归还，避免每次 SQL 都新建物理连接。池化管的是「可复用资源」（连接、线程等）；缓存管的是「可重复读的数据」，两者不是一回事。
- `MyBatis` / `MyBatis-Plus`：`MyBatis` 把 Mapper 方法与 SQL（XML 或注解）绑定，并把结果映射成 Java 对象；`#{}` 做参数绑定，`${}` 是字符串替换（仅白名单场景）。`MyBatis-Plus` 在其上补通用 CRUD、分页等，不取代理解 SQL 与映射。细节对象入口见 `Mapper`。
- `Transaction` / `@Transactional` / 事务：一组 SQL 要么都成功要么都回滚。Spring 里多用 `@Transactional` 声明边界，默认运行期异常回滚；事务通常挂在 Service，且依赖代理生效（同类自调用、`private` 方法等场景易失效）。同一事务共用同一连接。
...
- `cron` / `@Scheduled` / 调度平台：日历式定时。Unix crontab 是 5 段（分 时 日 月 周）；Spring / Quartz 的 `@Scheduled` cron 是 6 段（秒 分 时 日 月 周），`?` 表示该域不指定。XXL-JOB 等是带控制台的分布式调度。多实例重复执行、幂等、超时见[定时任务](#定时任务)。
...
3. 构建、依赖和部署怎么串起来
    ...
    - `Actuator`：短义见上文「名词解析」；流程里用来看健康检查和指标，生产注意权限。
...
7. 一次接口请求怎么走
    ...
    - `DispatcherServlet` 按 URL 找到 Controller；Controller 收参、调 Service、返回对象。
    - `Service` 做业务和事务；`Mapper` / `Repository` 访问数据库。
...
11. 数据库、SQL 和事务的主流程
    ...
    - `ACID`：短义见上文「名词解析」；多表写操作时尤其要关心失败会不会留下半成品。
    - `Index`：索引决定查询是否快；分页、排序、模糊搜索、唯一约束都要关注索引设计。

12. 事务怎么生效（及常见失效）

    `调用带 @Transactional 的 Spring Bean 方法 -> 代理拦截 -> 开事务/绑连接 -> 执行业务与 SQL -> 正常 commit / 异常 rollback`

    - 注解通常打在 Service public 方法；事务通过 **代理** 生效。
    - 同类里 `this.xxx()` 自调用、`private` / `final` 方法、异常被吞掉再抛受检异常且未配置 rollbackFor：都可能导致「以为有事务其实没有」。
    - 多数据源、异步线程要另看传播行为与连接是否还在同一事务里。

13. 缓存、消息和服务间调用

    - `Redis` / `TTL` / `MQ` / `ACK`·`NACK`·`DLQ`：短义见上文「名词解析」。这里只记：缓存看命中与三大问题；消息看确认、重试与死信。
