...
1. [行业术语](#行业术语)
    ...
    1. [自绘](#自绘)
    1. [Pipeline as Code原则](#pipeline-as-code原则)
1. [端口](#端口)
...
### Pipeline as Code原则
Pipeline as Code（流水线即代码）原则：通过`编码`而非~~配置~~持续集成/持续交付（CI/CD）运行工具的方式定义部署流水线。CI配置文件也应该如同项目源代码一样，存放在项目仓库中。这意味着配置文件也是纳入Git版本管理的，可以被其他人提MR修改，修改历史也很容易追溯追溯。在开源共建场景，这十分重要，因为协同开发者不仅要看到你的源代码，还需要知道你的构建流程。
