...
| ---------- | ---------------------------------- | --------------------------------------------------- |
| 所属层面 | 后端架构 | 前端架构 |
| 核心思想 | 将单体后端拆分成多个独立的服务 | 将单体前端拆分成多个独立的子应用 |
| 目标 | 提高后端可维护性、可扩展性、部署灵活性 | 提高前端可维护性、可协作性、渐进重构能力 |
| 拆分方式 | 按业务领域划分（用户、订单、支付等） | 按页面或功能模块划分（商品页、订单页、分析页等） |
| 技术栈 | 不同服务可使用不同语言/框架（如 Java、Go、Node.js） | 不同子应用可使用不同前端框架（如 React、Vue、Angular） |
| 通信方式 | 服务间通过 HTTP、RPC、消息队列等通信 | 子应用间通过全局事件、共享状态、postMessage 通信 |
| 数据管理 | 各服务有独立数据库（数据库隔离） | 各子应用管理自己的前端状态（如 Vuex、Redux），可共享全局状态 |
| 部署方式 | 各服务独立容器化部署（如 Docker、K8s） | 各子应用独立构建、独立上线，主应用动态加载 |
| 集成方式 | API Gateway（网关）统一入口 | 主应用（Shell/App Container）统一加载各子应用 |
| 版本升级 | 单个服务可独立升级 | 单个子应用可独立更新，不影响整体页面 |
| 团队协作 | 按业务服务拆分团队（如订单组、支付组） | 按页面或功能模块拆分团队（如用户中心组、报表组） |
| 优点 | 高扩展性、独立部署、容错性强 | 独立开发、技术灵活、渐进重构、团队并行 |
| 缺点 | 分布式系统复杂度高、数据一致性难 | 构建集成复杂、性能略受影响、样式与状态隔离需处理 |
| 代表技术 | Spring Cloud、Dubbo、gRPC、Kubernetes | qiankun、single-spa、Module Federation、Web Components |
| 典型使用场景 | 大型后端系统（电商、金融、SaaS 平台） | 大型前端门户（后台管理系统、多团队协作项目） |
...
3. [RxJS](https://github.com/ReactiveX/rxjs)
    ...
    1. ​响应式编程​：将程序看作是一系列的数据流，并通过定义数据流之间的依赖关系和操作来实现数据的处理和变化。
