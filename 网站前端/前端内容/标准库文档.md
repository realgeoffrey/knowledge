...
14. 模块Module
    ...
    - 概述
        ...
        1. 模块在严格模式`"use strict";`下运行（JS和`<script>`都是）。
        2. 模块要以路径`/`、`./`、`../`开头，否则要配合打包、转译工具。`.js`后缀可以省略。
        3. `export`和`import`可以放在模块的任何位置，但必须处于模块顶层；若处于~~块级作用域内~~会报错（无法进行条件判断或按需加载）。
        4. 编译阶段进行模块加载（静态分析）。
        5. `default`可以当作默认变量来看待。
        6. 导入的变量与原模块导出的变量是**动态绑定**，可以实时获取原模块内部的值，因此多个脚本导入同一模块的值是相同的引用。导入的变量是只读，不能重新赋值，但可以修改导出的变量的属性（与`const`变量类似）。

            >e.g.
            >
            >```javascript
            >// moduleA
            >export var foo = {a: 'first'};
            >setTimeout(() => foo.a = 'second', 500);
            >
            >// moduleB
            >import {foo} from './moduleA'
            >// foo的引用的值一开始是{a: 'first'}，500ms之后会动态改变为{a: 'second'}（模块加载需要考虑时间）
            >// 可以修改、添加foo的属性，但是不能对foo进行赋值
            >```
    ...
    1. `export`
        ...
        - 特性：
            ...
            1. 模块中所有声明相对于模块而言都是本地作用域（`"use strict";`），必须导出才能让其它模块导入本模块内容（或`window.属性`，不推荐）。
            2. 模块可以导出多个内容，导出变量名不能重复（因此`default`只能导出一次）。
        ...
        4. `export {变量[, 变量]}或{变量 as 外号变量[, 变量 as 外号变量]}或* form '模块'`
            ...
            >`export * form '模块'`不会导出模块的`default`，可以用`export { default } from '模块'`导出`default`。
    ...
    2. `import`
        ...
        - 特性：
            ...
            2. 导入的变量会被提升（hosting）。
            3. （CORS）跨域的模块资源无法`import`引入，也无法通过`<script>`以模块方式加载。
