...
1. `static`属性/方法直接是类的属性；类内部定义的所有方法都在`类.prototype`；类内部定义的属性是实例的属性。
2. 类内部定义的属性/方法都不可枚举（类内部定义的静态属性是可枚举）、且方法都在[严格模式](https://github.com/realgeoffrey/knowledge/blob/master/网站前端/前端内容/标准库文档.md#严格模式)下运行。
3. 类内部的方法（尤其是`constructor`）内能新增实例的属性/方法、或类的原型属性/方法。

    1. `this.新增属性 = 新值`：在实例增加属性/方法。
    2. `Object.getPrototypeOf(this).新增属性 = 新值`：在原型链上增加属性/方法。
4. 类声明完毕之后：
