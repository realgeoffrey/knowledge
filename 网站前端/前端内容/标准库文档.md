...
3. 浮点数的计算都可能导致结果出现奇怪小数、浮点数的四舍五入都可能不准确
    ...
    - 运算问题解决：①放大倍数后成为整数，代替小数进行四则运算，最后除以放大倍数；或②按位计算后，再判断小数点位置。

        >整数相除可以避免运算时误差，但展示小数的舍入误差无法避免。
...
2. Number原型链（继承给所有实例）：
    ...
    1. `Number.prototype.toFixed([数字])`
        ...
        >[decimal.js](https://github.com/MikeMcl/decimal.js)：`toSignificantDigits`保留多少位有效位数（若小数末尾为零则去除——不补零）、`toFixed`保留多少位小数位（必定返回设定的小数位数——补零）、`toDecimalPlaces`至多保留多少位小数位（若小数末尾为零则去除——不补零）。
...
>- 「在A时区存储时间，在A或其他时区展示，展示的是正确的绝对时间」的解决方案——不同时区展示的时间换算成时间戳是相等的：
>
