...
1. 调用构造函数：
    ...
    1. `new Date([毫秒/时间字符串/年, 月[, 日[, 时[, 分[, 秒[, 毫秒]]]]]])`
        ...
        >e.g. 时间字符串若包含时分秒毫秒，则表示当前系统时区的偏移量，否则表示UTC+0的偏移量，e.g. `new Date('2019-10-10 0:0:0')` => 2019-10-10 0:0:0 UTC±当前系统时区的偏移量（等价于`moment('2019-10-10')._d`）；`new Date('2019-10-10')` => 2019-10-10 0:0:0 UTC+0（等价于`moment.utc('2019-10-10')._d`）。
