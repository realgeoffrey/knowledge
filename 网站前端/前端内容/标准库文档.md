...
1. [ES6](#es6)
    ...
    1. [ECMAScript 2021新增](#ecmascript-2021新增)
...
2. Number原型链（继承给所有实例）：
    ...
    1. `Number.prototype.toFixed([数字])`
        ...
        1. 描述：指定小数点后数字个数（默认：`0`），省去的四舍五入，没有的小数位置补`0`（e.g. `(1.1).toFixed(2) === '1.10'`）。
        2. 返回：一个字符串。

        >```javascript
        >// 保留小数点后n位，四舍五入，位数不够，不补0，返回数字类型
        >function formatFloat (num, places = 0) {
...
>2. 用`<img>`代替 ~~`background-image`等css背景~~，解决图片模糊问题。
...
3. 获取Base64值
    ...
    >其他：canvas实例`.toDataURL`方法返回Base64值。
...
1. 字符串替换：
    ...
    >e.g. `console.log("1、 %o 2、 %s 3、 %d 4、 %f 5、 %c带颜色%c关闭CSS", { a: 1, b: false, c: [], d: { e: 1 } }, "字符串", 1.1, 1.1, "color: yellow; font-style: italic; background-color: blue; padding: 2px", "")`
