...
4. 相对于非严格模式，增加的限制（违背则抛出错误）：
    ...
    3. 安全
        ...
        4. 没有调用对象的直接调用时，`this`指向`undefined`而不指向~~全局对象~~，除非被`call/apply/bind`后`this`为传入的值。
            ...
            >在非严格模式，若是基本数据类型（除了`undefined`、`null`之外）使用`call/apply/bind`，则`this`成为那个值的包装对象。
    4. 为未来的ES铺平道路
...
2. `this`
    ...
    不会创建自己的`this`（与`function`关键字不同），而使用封闭执行上下文最近的一个`this`值。因此对于`call/apply/bind`等改变`this`指向的方法也无法改变箭头函数体内使用的`this`。

    >原因：箭头函数并不会绑定`this`变量（`arguments`、`super`、`new.target`类似），`this`、`arguments`、`super`、`new.target`的值（作用域）会如同寻常所做的一样一层层地去往上查找。
