...
>给一个对象直接设置属性，则此属性为数据属性，`configurable`、`enumerable`、`writable`为`true`，`value`为直接设置的属性值。
...
2. 访问器属性：
    ...
    3. `get`（默认：`undefined`）
        ...
        属性的getter方法，该方法返回值为对象的该属性值。
    4. `set`（默认：`undefined`）

        属性的setter方法，该方法接受一个参数，参数值为对象的该属性赋值符号右边的值。

    ><details>
    ><summary>不用<del>defineProperty</del>或<del>defineProperties</del>的写法</summary>
    >
    >```javascript
    >var obj = {
    >  get foo () {
...
2. Function原型链（继承给所有实例）：
    ...
    >因为箭头函数没有创建自己的this，而是向上查找上级作用域的this，所以`call/apply/bind`无法指定箭头函数使用的this（this修改无效），但仍然可以添加形参。
...
4. 相对于非严格模式，增加的限制（违背则抛出错误）：
    ...
    3. 安全
        ...
        6. 被`call/apply/bind`后`this`为传入的值（不会转化为~~基本包装类型~~；传入`null/undefined`就为`null/undefined`）。

            >在非严格模式，若是基本数据类型（除了`undefined/null`之外）使用`call/apply/bind`，则`this`成为那个值的基本包装类型；不传或传入`undefined/null`，则`this`为全局对象。
    4. 为未来的ES铺平道路
...
1. `let`、`const`（与`var`的区别）：
    ...
    1. （除了全局、函数作用域之外，）拥有块级作用域（声明变量仅在块级作用域中有效，离开块级作用域删除）。
        ...
        2. `with(){/* 块级作用域 */}`
        3. `if(){/* 块级作用域 */}`
        4. `for(/* 块级作用域（包括for、for-in、for-of） */){/* 块级作用域 */}`
        5. `while(){/* 块级作用域 */}`、`do{/* 块级作用域 */}while()`
        6. `try{/* 块级作用域 */}catch(e){/* 块级作用域 */}finally{/* 块级作用域 */}`
        7. `switch(){/* 块级作用域 */}`

        ><details>
        ><summary>ES6之前可以用的块级作用域：<code>with</code>（不安全、不建议使用）、<code>try-catch</code></summary>
        >
        >```javascript
        >{                  // ES6的块级作用域
        >  let a = 2
...
2. 不会创建箭头函数自己的`this`、`arguments`、`super`、`new.target`，根据[词法作用域](https://github.com/realgeoffrey/knowledge/blob/master/网站前端/程序员的自我修养/README.md#词法作用域动态作用域)的方式向上遍历查找。
    ...
    1. `this`
        ...
        不会创建自己的`this`（与`function`关键字、简写的方法不同），而使用封闭执行上下文最近的一个`this`值。因此对于`call/apply/bind`等改变`this`指向的方法也无法改变箭头函数体内使用的`this`。
...
1. 解构赋值（destructuring assignment）
    ...
    >1. 浅复制。
    >2. 针对对象，解构自身和整条原型链上的属性（除了解构的剩余参数模式仅解构自身的属性）。
...
1. 作为属性键
    ...
    1. 不被大部分遍历方法返回：
        ...
        `for-in`、`Object.entries/values/keys/getOwnPropertyNames`、`JSON.stringify`、`Reflect.ownKeys`
    2. 能够返回：

        `Object.getOwnPropertySymbols`、对象的展开元素、Map类型的遍历
2. 不可更改。
...
#### `Performance`
>参考：[MDN：Performance](https://developer.mozilla.org/zh-CN/docs/Web/API/Performance)。
