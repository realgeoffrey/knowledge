...
2. 浮点数的计算可能导致结果出现奇怪小数、浮点数的四舍五入都可能不准确
    ...
    浮点数的计算或四舍五入之后，不要进行精确的等于/不等于比较，容易出纰漏。
...
2. `JSON.stringify(JS值[, 处理方法或需要的属性名组成的数组[, 缩进空格数量或缩进字符串]])`
    ...
    >3. 值为：`Set`、`WeakSet`、`Map`、`WeakMap`类型的值，转化为`{}`。
    >4. 值为：`undefined`、`方法`、`Symbol`类型的值
    >
    >    1. 非数组中：属性名-值整个被忽略
    >    2. 数组中：被转换成`null`
    >5. 不支持`BigInt`类型的值（`TypeError: Do not know how to serialize a BigInt`）。
    >6. 所有`Symbol`类型的属性键都会被忽略。
    >7. 不可枚举的属性被忽略。
...
#### `Reflect`
操作对象的默认行为。Reflect旨在补充Proxy，对于任意Proxy捕捉器，都有一个带有相同参数的Reflect调用。应该使用它们将调用转发给目标对象。
...
4. 新增：Error对象的`cause`属性，可用于记录错误的因果链；
    ...
    >```
    ></details>
5. 新增：Strings、Arrays、TypedArrays的`at`方法，支持正整数 或 负整数索引（若给出正整数索引，则这种新方法与使用括号表示法访问具有相同的行为；若给出负整数索引，则就会从数组的最后一项开始检索）；
...
#### Canvas
>来自：[MDN:Canvas](https://developer.mozilla.org/zh-CN/docs/Web/API/Canvas_API)。
