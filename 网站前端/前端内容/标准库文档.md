...
4. 相对于非严格模式，增加的限制（违背则抛出错误）：
    ...
    3. 安全
        ...
        4. 没有调用对象的直接调用时，`this`指向`undefined`而不指向~~全局对象~~，除非被`call/apply/bind`后`this`为传入的值。
            ...
            >1. 在非严格模式，若是基本数据类型（除了`undefined`、`null`之外）使用`call/apply/bind`，则`this`成为那个值的包装对象。
            >2. 在严格模式，箭头函数的没有调用对象的直接调用时，`this`依然指向全局对象。
    4. 为未来的ES铺平道路
...
2. 不创建箭头函数自己的`this`、`arguments`、`super`、`new.target`，在函数调用时往作用域链上一层查找继承。
3. `this`
...
4. 不能作为~~构造函数~~（不能对箭头函数使用`new`）。
5. 不能用作生成器（不能在箭头函数方法体内使用`yield`）。
6. 没有原型属性`prototype`。
...
1. 解构赋值（destructuring assignment）
    ...
    3. 基本数据类型的解构：
        ...
        2. 其他类型
            ...
            1. `[变量]=其他类型`，除了字符串（可迭代对象）之外报错`TypeError: undefined is not a function`，字符串按顺序每一个字符赋值给每一个变量。
...
3. Promise原型链（继承给所有实例）
    ...
    3. `Promise.prototype.finally(函数)`（ES9）
        ...
        无论Promise实例最终状态如何（完成/失败）都会执行函数。不接受任何参数、无法知道Promise实例的完成/失败状态；回调函数`return`的值只有失败状态才作为Promise实例向下链式传递，否则向下链式传递的上一个链式的Promise实例（上一个状态可以是完成/失败）。
