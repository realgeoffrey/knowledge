...
5. `history`
    ...
    2. 方法
        ...
        4. `history.pushState(state对象, 标题字符串, URL字符串)`
            ...
            >不触发页面刷新、不触发`hashchange`事件，若设置跨域网址则报错。
    ...
    - 额外
        ...
        window的`popstate`事件：文档的`history`出现变化时触发（调用`pushState`或`replaceState`时不触发，页面第一次加载时不触发。点击浏览器倒退或前进按钮时触发，调用`history`的`back/forward/go`时触发）。
6. `Blob`
