...
2. Object原型链（继承给所有实例）：
    ...
    5. `Object.prototype.toLocaleString()`
        ...
        1. 描述：调用`toString`。用于被派生的对象重写，如：`Number`、`BigInt`、`Date`、`Array`、`「TypedArrays」`、有重写`toLocaleString`逻辑，没有重写的原型会直接调用其原型的`toString`。

            >1. `Number或BigInt.prototype.toLocaleString(2参数)`等价：`Intl.NumberFormat(2参数).format(num)`
            >1. `Date.prototype.toLocaleString(2参数)`（`toLocaleDateString`、`toLocaleTimeString`同理）等价：`Intl.DateTimeFormat(2参数).format(date)`
            >1. `Array或「TypedArrays」.prototype.toLocaleString(2参数)`等价：遍历元素，每个元素触发其`toLocaleString`（「TypedArrays」都是数字，所以其遍历后必然调用`Intl.NumberFormat(2参数).format(num)`）
            >1. `Object.prototype.toLocaleString()`（以及其他没有重写的类型）等价：`Object.prototype.toString()`
        2. 返回：一个字符串。
...
3. Array原型链（继承给所有实例）：
    ...
    1. 改变调用对象（mutator方法）：
        ...
        6. `Array.prototype.sort([排序方法])`
            ...
            >    })
            >    ```
            >6. 当排序非ASCII字符的字符串（如：包含类似 e、é、è、a、ä、中文 等字符的字符串）需要使用`String.prototype.localeCompare`将函数排序到正确的顺序。
            >
...
#### 解构赋值、默认参数、剩余参数、展开元素
>默认参数用`=`设置默认值，对象的解构用`:`换变量名；Typescript用`:`定义形状。所有涉及新变量创建和赋值都是[值传递](https://github.com/realgeoffrey/knowledge/blob/master/网站前端/JS学习笔记/README.md#内存机制)。所有剩余参数都遵循：只能有一个，并且必须放在参数列表的最后面。
...
4. 新增：支持函数参数列表和调用中的尾逗号Trailing comma（e.g. `function func(a, b, c,) {}; func(1, 2,)`）
...
2. 方法
    ...
    >2. `标题字符串`：历史兼容参数，现代浏览器通常会忽略这个值；可填空字符串或`null`。
