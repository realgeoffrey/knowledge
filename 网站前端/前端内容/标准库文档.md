...
1. 执行步骤：
    ...
    3. `async`返回Promise对象（若返回非Promise对象，则返回值类似包裹着`Promise.resolve(返回值)`），在函数体返回前，是未完成状态；
    4. 返回：
...
/* 使用测试 */
a().then(() => {}, () => {})    // async方法返回的是Promise对象；若方法体内抛出错误则状态失败，否则状态完成（包括默认返回的undefined）；返回值或抛出异常值为向下传递的值
```
</details>
