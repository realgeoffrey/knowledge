...
2. Array原型链（继承给所有实例）：
    ...
    3. 改变调用对象（mutator方法）：
        ...
        9. `Array.prototype.fill(填充的值[, 开始索引[, 结束索引]])`
            ...
            1. 描述：用填充的值去填充数组中从起始索引到终止索引（不包括）的元素。
            2. 返回：修改后的数组。
...
9. 箭头函数
    ...
    2. `this`
        ...
        不会创建自己的`this`（与`function`关键字不同），而使用封闭执行上下文最近的一个`this`值。因此对于`bind/call/apply`等改变`this`指向的方法无法改变箭头函数内使用的`this`。

        >原因：箭头函数并不会绑定`this`变量，它的作用域会如同寻常所做的一样一层层地去往上查找。对于`arguments`、`super`、`new.target`也同样如此。
        ...
        function foo1() {    // 创建this
          return () => {     // 不创建this
            return () => {   // 不创建this
              return () => { // 不创建this
                console.log('1:', this.id1); // this指向foo1方法的this
              };
            };
          };
        }
        ...
        var foo2 = () => {    // 不创建this
          return () => {      // 不创建this
            return () => {    // 不创建this
              return () => {  // 不创建this
                console.log('2:', this.id2);  // this指向全局对象window
              };
            };
          };
        };
    ...
    4. 不能作为构造函数（不能对箭头函数定义的方法使用`new`）。
    5. 不能用作生成器（不能在箭头函数定义的方法内使用`yield`）。
    6. 没有原型属性`prototype`。
10. `Symbol`
