...
6. JS引擎（JS解释器）
    ...
    >JS引擎提供了执行JavaScript的运行时环境。DOM和其他Web平台API则由浏览器提供（，Node.js模块由Node.js提供）。JS引擎都实现了[Standard ECMA-262](https://www.ecma-international.org/publications/standards/Ecma-262.htm)（ECMAScript：JavaScript使用的标准）。
