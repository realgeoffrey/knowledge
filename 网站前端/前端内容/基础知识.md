...
>    1. 标量（scalar）
>
>        单一数值：字符串（`String`）或数字（`Number`）或布尔值（`Boolean`：`true`、`false`）或Null（`null`）。
>    2. 序列（sequence）
>
...
4. JSON的发展
    ...
    3. [JSON5](https://github.com/json5/json5)
        ...
        JSON5 的目标是完全对齐 ECMAScript 5.1 的语法。

        | 特性类别 | 具体特性 | 标准 JSON | JSON5 | JSON5 代码示例与说明 |
        | --- | --- | --- | --- | --- |
        | 注释 (Comments) | 单行注释 | ❌ 严禁使用 | ✅ 支持 | `// 这是一个单行注释` |
        | | 多行注释 | ❌ 严禁使用 | ✅ 支持 | `/* 这是一个多行注释 */` |
        | 对象 (Objects) | 键名 (Key) 引号 | ❌ 必须使用双引号包裹 | ✅ 合法标识符可省略引号 | `{ name: "Alice", "first-name": "Alice" }`（带横杠等特殊字符仍需引号） |
        | | 尾随逗号 | ❌ 不允许 | ✅ 允许在最后一个键值对后加逗号 | `{ a: 1, b: 2, }`（方便 Git 版本控制） |
        | 数组 (Arrays) | 尾随逗号 | ❌ 不允许 | ✅ 允许在最后一个元素后加逗号 | `[ 1, 2, 3, ]` |
        | 字符串 (Strings) | 引号类型 | ❌ 只能使用双引号 `"` | ✅ 支持单引号 `'` 和双引号 `"` | `'单引号字符串'` 或 `"双引号字符串"` |
        | | 多行字符串 | ❌ 必须写在一行，用 `\n` | ✅ 支持用反斜杠 `\` 跨行书写 | `'第一行 \第二行'`（视觉换行，实际解析为一行） |
        | | 单引号转义 | ❌ 不适用 | ✅ 支持 `\'` 转义 | `'It\'s JSON5'` |
        | | 十六进制表示字符 | 只能用 4 位的 Unicode 转义，例如 \u0041（表示字母 A） | 除了支持 \u，还额外支持 JS 原生的 2 位十六进制转义，例如 \x41（同样表示字母 A），<br>并且允许在字符串中转义真实的换行符（不仅仅是视觉换行的反斜杠 \） |  |
        | 数值 (Numbers) | 正号前缀 | ❌ 仅支持负号 `-` | ✅ 支持显式加正号 `+` | `+123`, `+3.14` |
        | | 十六进制 | ❌ 仅支持十进制 | ✅ 支持 `0x` 或 `0X` 开头 | `0x1A`, `0xdecaf` |
        | | 前导/尾随小数点 | ❌ 必须以数字开头和结尾 | ✅ 允许省略小数点前后的 0 | `.5`（即 0.5）、`5.`（即 5.0） |
        | | 特殊数值常量 | ❌ 不支持 | ✅ 支持 NaN（非数字） | `NaN`, `+NaN`, `-NaN` |
        | | 无穷大常量 | ❌ 不支持 | ✅ 支持 Infinity | `Infinity`, `+Infinity`, `-Infinity` |
        | 空白字符 (Whitespace) |  | 空格、水平制表符 (`\t`)、换行 (`\n`)、回车 (`\r`) | +垂直制表符 (`\v`)、换页符 (`\f`)、不换行空格 (`NBSP`) 等所有 ECMAScript 5.1 定义的空白字符 |  |

    4. JSONC（JSON with Comments）

        >不是官方标准组织发布的规范，而是由于实际开发需求演变出的一种事实标准。

        标准JSON + 单行/多行注释 + 尾随逗号
    5. [JSON-RPC](https://www.jsonrpc.org/specification)

        1. 无状态、轻量级RPC协议，数据格式为JSON，传输层无关（可走HTTP、WebSocket等）
        1. 通常单一URL，根据`method`区分功能，无法利用HTTP缓存
        1. 请求：`{"jsonrpc": "2.0", "method": "方法名", "params": 参数, "id": 字符串|整数|不传}`（不传`id`为通知，服务端不响应）
        1. 成功响应：`{"jsonrpc": "2.0", "result": 结果, "id": 同请求}`
        1. 错误响应：`{"jsonrpc": "2.0", "error": {"code": 整数, "message": "错误信息", "data": 可选}, "id": 同请求|null}`（`id`为null表示在Request对象中检测到ID时出现错误，例如解析错误/无效请求）
        1. 支持 Batch：请求/响应可为数组，批量调用

        | 特性   | RESTful API | JSON-RPC |
        |--------|-------------|----------|
        | 设计   | 面向资源     | 面向方法 |
        | URL    | /users/123/orders | 通常单一入口如 /api |
        | HTTP   | GET/POST/PUT/DELETE | 通常全用 POST |
        | 错误   | HTTP 状态码  | 200 + JSON 内 error |
        | 场景   | CRUD 公开 API | 微服务、Web3、游戏后端、区块链通信 |
    6. [JSON Lines](https://jsonlines.org/)

        每行一个完整 JSON 值（通常为对象），无外层`[]`、无行间逗号，仅用换行`\n`（或`\r\n`）分隔。扩展名：`.jsonl`、`.ndjson`。常见于流式 API、日志、大表/机器学习 数据集。

        **格式要点**（相对 JSON 的额外约定）：空行无效、不能尾逗号（每行须为合法 JSON）；行内不能换行；建议最后一行也以`\n`结尾。大文件可用 gzip → `.jsonl.gz`。MIME：`application/jsonl`（未正式标准化）。

        | 对比项     | 标准 JSON | JSONL |
        |------------|-----------|--------|
        | 流式读取   | 需整文件入内存，大文件易 OOM | 逐行读、解析、丢弃，内存恒定 |
        | 追加写入   | 需改末尾`]`、补逗号，易破坏结构 | 追加模式直接写一行即可 |
        | 局部损坏   | 一处语法错误全文件解析失败 | 坏行 try/catch 跳过，其余照常解析 |
...
- JSON流问题的解决：<https://github.com/ndjson/ndjson-spec>
