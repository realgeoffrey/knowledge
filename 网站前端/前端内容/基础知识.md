...
- 在非严格模式，若是基本数据类型（除了`undefined/null`之外）使用`call/apply/bind`设置`this`，则`this`成为那个值的基本包装类型。
...
3. 字符串
    ...
    // 构造函数实例化
    var str3 = new String('string');    // 基本包装类型
...
- 其他基本数据类型（除了 ~~`Undefined`~~、~~`Null`~~），也有字面量方式和构造函数（或普通函数）方式。
