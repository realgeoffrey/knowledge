...
1. 使用`vm`（或`vh`）
    ...
    >参考：[大漠：再聊移动端页面的适配](https://fedev.cn/css/vw-for-layout.html)。
    ...
    >    300px的设计稿距离计算如下
    >
    >    1. 1080px设计稿中300px占用：`300px / 1080px ≈ 27.8%`；
    ...
    >    3. `27.8vw === 设计稿宽度的27.8%`，从而设计稿对应了响应式页面。
    ></details>
2. <details>
    ...
    2. **媒体查询**设置**不同布局视口宽度**下**html的font-size值**，把要做成响应式的内容转换为rem单位
        ...
        >0.5px问题用[WAP端半像素](https://github.com/realgeoffrey/knowledge/blob/master/网站前端/HTML+CSS学习笔记/实现具体业务.md#wap端半像素)处理，或直接设置为1px（无视像素级还原）。
    </details>
