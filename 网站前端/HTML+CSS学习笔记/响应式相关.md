...
2. Android
    ...
    360dp总宽（还有sp单位），设备无关的逻辑像素，保证资源（图片）有`1x图`、`1.5x图`、`2x图`、`3x图`、`4x图`。
3. React Native、Flutter、小程序、其他语言2Native

    设备无关的逻辑像素。

>客户端根据设备分辨率情况，去选择对应的N倍图，若取不到则自动降级取低倍图。

4. 前端

    >px不是响应式。

    rem（需要font-size支持）、vw（vh）也是设备无关的逻辑像素的考虑。
5. 没有支持响应式的场景，只能计算出响应式
