...
1. [各端的还原设计稿方案（响应式方案）](#各端的还原设计稿方案响应式方案)
1. [基本概念、术语](#基本概念术语)
1. [px转rem方案](#px转rem方案)
1. [响应式设计](#响应式设计)
...
1. iOS
    ...
    >所有机型的逻辑像素总宽度都是375固定不变。

    375（无单位）总宽，设备无关的逻辑像素，保证资源（图片）有`@2x图`、`@3x图`。
2. Android

    >各机型的dp总宽度范围很广，主要有：320~414、800+，最多的是360。

    <details>
    <summary>Android资源图</summary>

    ![Android图](./images/android-picture.png)
    </details>

    （大部分情况下）360dp总宽，设备无关的逻辑像素，主要利用flex布局，保证资源（图片）有`1x图`、`1.5x图`、`2x图`、`3x图`、`4x图`。

>设备根据分辨率情况，去选择对应的N倍图，若取不到则自动降级取低倍图。

3. 设备无关的逻辑像素（作为配合flex布局的补充）

    1. 小程序

        默认配置规定屏幕总宽为750rpx。
    2. React Native、Hippy、等

        实现类似rem的响应式计算方法。
    3. Flutter利用[flutter_screenutil](https://github.com/OpenFlutter/flutter_screenutil)
...
- 总结：
    ...
    1. 各端响应式方案：
        ...
        获得设备视口宽度，根据设计稿宽度，进行换算后还原设计稿：主要是`flex布局`，特别要求的再进行`像素级还原`。

        1. 若有些系统的设备视口宽度固定不变（如：iOS、小程序），则能直接进行像素级还原设计稿，也能使用flex布局。
        2. 若有些系统的设备视口宽度随着设备型号而不同（如：Android、React Native），则：

            1. 遵循某主要设计稿宽度（如：375） + flex布局（子级：固定宽度 + 自适应拉伸填满父级）。
            2. （像素级还原设计稿）实现类似rem的响应式计算方法：`设计稿中某长度 / 设计稿总宽 * 设备视口宽度`。
    2. 通用的还原设计稿方案：

        1. 设计稿一般输出的是主要针对的某个设备视口宽度，其他宽度需要开发人员进行自适应处理。

            >若是设备视口宽度固定不变的系统（如：iOS、小程序），则任何数值都是像素级还原设计稿，也就不需要处理自适应问题。
        2. 使用flex布局还原设计稿。在flex子项中：

            >保证宽度横向铺满自适应。

            1. 判断出一部分节点用固定宽度（如：图片、明显不是自适应的部分）
            2. 另一部分拉伸填满空间（弹性布局）
        3. 针对`padding`、`margin`、`border`、`font-size`、`图片`、等，一般写死数值、不自适应变化。
        4. 对于特别要求或特别适合像素级还原设计稿的部分，再用类似rem的响应式计算方法。
