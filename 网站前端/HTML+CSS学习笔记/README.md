...
1. `auto`（默认）
    ...
    1. 在`<td>`或`<th>`上设置的宽度为“建议宽度”，但内容优先。没有设置宽度的项平分父级余下的宽度。若所有项都设定宽度则所有项内容占满之后按（某个逻辑）比例分父级余下的宽度。
    2. 内容撑开`<table>`
2. `fixed`

    >整个表格在首行被下载后就被解析和渲染。更快完成渲染，性能更优。

    1. 第一行`<th>`或`<td>`（或第一行`<col>`、`<colgroup>`）宽度决定所有行宽度。设置的宽度为强制宽度，忽略内容宽度。没有设置宽度的项平分父级余下的宽度。若所有项都设定宽度则所有项按比例分父级余下的宽度。
    2. 内容不撑开`<table>`
