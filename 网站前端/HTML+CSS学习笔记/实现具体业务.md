...
1. [让图片和背景融为一体](#让图片和背景融为一体)
1. [“聚光灯”效果（Spotlight Effect）](#聚光灯效果spotlight-effect)
...
1. 使用`display: flex;`
    ...
    - 仅有唯一的Flex子项，设置`margin: auto`可以使其垂直水平居中
...
### `filter`滤镜
>若仍需兼容 IE10/IE11，没有通用 CSS filter 方案；现代主流浏览器基本支持。
...
### 让图片和背景融为一体
1. 方案一：在图片后面叠加一个相同图片的放大后高斯模糊图片为背景
    ...
    filter: blur(10px);
    ```
2. 方案二：运用js库获取图片边缘颜色，设置为背景色

### “聚光灯”效果（Spotlight Effect）
```html
<style>
  /* 页面底色设为黑色，为了看清亮光 */
  body {
  ...
  /* *核心光圈层：在卡片内容下层创建一个圆形的渐变背景，圆心在鼠标坐标处，其卡片内容（.inner）的背景会覆盖掉渐变背景，仅留出边框距离的渐变背景，导致看起来像光圈 */
  .card::before {
    content: "";
    position: absolute;
    inset: 0;
    ...
    background: radial-gradient(
      300px circle at var(--x, -999px) var(--y, -999px),
  ...
  /* *鼠标进入网格时，显示所有卡片的光圈（实现联动） */
  #grid:hover .card::before {
    opacity: 1;
  }

  /* *卡片内容（遮罩父级背景，仅留出边框距离的渐变背景，导致看起来像光圈） */
  .inner {
    position: absolute;
    inset: 1px; /* 关键：往里缩 1px，把底层光效当做边框漏出来 */
    background: #111; /* 卡片本身的深色背景。若去掉当前这个背景，则光圈会直接显示在卡片内容上而不是边框上 */
    border-radius: 9px; /* 外层 10px - 1px = 9px */
...
<div id="grid">
  <div class="card"><div class="inner">卡片 1 尝试hover效果</div></div>
  <div class="card"><div class="inner">卡片 2 尝试hover效果</div></div>
  <div class="card"><div class="inner">卡片 3 尝试hover效果</div></div>
  <div class="card"><div class="inner">卡片 4 尝试hover效果</div></div>
  <div class="card"><div class="inner">卡片 5 尝试hover效果</div></div>
</div>
...
[CodePen demo](https://codepen.io/realgeoffrey/pen/emdYQwo)
