...
1. `v-model`
    ...
    >const model1 = modelModifiers()                // 无论传不传修饰符，model1都是值
    >const [model1, modifiers1] = modelModifiers()  // 无论传不传修饰符，model1都是值，modifiers1都是对象（空对象或{修饰符:true}）
    >```
    ...
    1. `v-model.修饰符`（除了`.trim`、`.number`、`.lazy`之外，也支持自定义修饰符）
        ...
        1. defineMode
            ...
            ```
        2. 传入子级的props增加`modelModifiers`对象：`modelModifiers.自定义修饰符: true或不存在`
    1. `v-model:xx.修饰符`
...
1. 全局注册组件`const app = createApp({}); app.component( 名, 组件选项 )`
...
1. `reactive()`只接受引用类型（创建Proxy对象以引用类型对象作为目标，具有响应性）；`ref()`可以接受任何值类型（包括组件或DOM，此时建议初始化为`ref(null)`或`rel([])`），会返回一个包裹对象，并在`.value`属性下暴露内部值（使`.value`的值具有响应性）。
...
1. 侦听器
    ...
    1. 副作用清理
      ...
      1. 触发时机：侦听器回调会在父组件更新 (如有) 之后、所属组件的 DOM 更新之前被调用。这意味着如果你尝试在侦听器回调中访问所属组件的 DOM，那么 DOM 将处于更新前的状态。
          ...
          修改调用时机：传参配置`flush: 'post'或'sync'` 或`watchPostEffect`、`watchSyncEffect`。
      1. 停止侦听器
...
1. `defineProps`、`defineEmits`、`defineExpose`、`defineModel`、`defineOptions`是一个编译器宏（compiler macro），并不需要导入（但也不能打印或赋值给其他变量），都是只能在`<script setup>`顶层中使用（不能从其他文件引用，与生命周期钩子不同）
...
4. <details>
    ...
    export default {
      ...
      mounted () {
        ...
        this.vm = new OtherVue({  // 后定义，类似于mixin的合并逻辑（钩子和watch：先定义执行 -> 后定义执行；其他：合并，后定义执行、先定义忽略）
...
- 避免问题
    ...
    3. 针对非自适应宽度的`<el-table>`，避免所有`<el-table-column>`都设置固定宽度，至少需要一个项不设置宽度或设置`min-width`。
    ...
    9. `<el-pagination>`需要使用`.sync`才会同步修改传入`page-size`、`current-page`的值（或事件触发后手动修改），并且 **事件`size-change`、`current-change`是组件手动操作后触发相关内部值变化后才会触发**，~~任何方式修改传入值的变化都不会触发`size-change`、`current-change`事件~~
