...
2. 断开连接的四次挥手
    ...
    1. 主机1发送`FIN=1,seq=主机1序号`的断开信息。
    2. 主机2接收消息，发送`ACK=1,ack=主机1序号+1`的响应消息。
    3. 主机2发送`FIN=1,seq=主机2序号`的断开信息。
    4. 主机1接收消息，发送`ACK=1,ack=主机2序号+1`的响应消息。
...
>2. 四次挥手原因：
>
>    TCP是**全双工模式**，当主机1发出FIN报文段时，只是表示主机1已经没有数据要发送了；但是，这个时候主机1还是可以接受来自主机2的数据；当主机2返回ACK报文段时，表示主机2已经知道主机1没有数据发送了，但是主机2还是可以发送数据到主机1；当主机2也发送了FIN报文段时，这个时候就表示主机2也没有数据要发送了，之后彼此都中断这次TCP连接。
...
2. POST
    ...
    向指定资源提交数据，请求服务器进行处理（例如提交表单或上传文件）。数据被包含在请求本文中。这个请求可能会创建新的资源或修改现有资源，或二者皆有。
3. HEAD
...
2. 结构：
    ...
    不区分大小写的属性名后跟一个冒号，再紧跟它的值（不必换行）组成，以回车（CR）加换行（LF）符号序列结尾。值前面的空格会被忽略掉。自定义专用消息头可通过`X-`前缀来添加（已废弃）。
3. 类型
    ...
    2. 请求头部字段（request）
        ...
        | If-Modified-Since | 比较资源的更新时间，之后时间有更新则服务器接受请求 |
        | If-Unmodified-Since | 比较资源的更新时间，之后时间没有更新则服务器接受请求 |
        | If-Range | 比较ETag或时间，相同则作为范围请求处理；反之，则返回全体资源 |
        | Range | 实体的字节范围请求 |
