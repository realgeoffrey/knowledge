...
2. HTTPS（HTTP Secure） = HTTP + 通信加密 + 证书认证 + 完整性保护
    ...
    1. 加密：共享密钥加密、公开密钥加密。
        ...
        2. 公开密钥加密（Public-key cryptography，非对称加密）：

            >处理速度较慢。

            发送密文的一方使用对方的公开密钥进行加密，对方收到被加密信息后，再使用自己的私有密钥进行解密。
...
- 若服务端配置允许了某些或所有域名，就可以进跨域响应；前端不需要进行额外工作，现代浏览器自动完成交互：
    ...
    现代浏览器一旦发现AJAX（或`fetch`）请求跨源（与请求的数据格式无关），会自动添加一些附加的头信息（如`Origin`），有时还会多出一次附加的请求（非简单请求的`OPTIONS`请求），但用户不会有感觉。
