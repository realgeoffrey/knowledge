...
1. `if 「条件」:`（没有小括号 ~~`()`~~ ）、`elif 「条件」:`、`else"`
    ...
    2. 支持`18.5 <= 变量 < 24`
1. `for 变量 in 列表或元组或集合或字符串或字典:`、`for 变量 in range(参数):`
...
1. 函数
    ...
    1. `print`
        ...
        4. `print(f'\033[0m...\033[0m')`
            ...
            控制输出内容的颜色
    1. `type(变量名或值)`检查类型
    1. `import math`、`import random`

        `from math import factorial`、`from math import factorial as f`
    ...
    1. `list()`将其他序列变成列表
        ...
        1. 列表是一种可变容器
        0. `[多个项]`直接创建字面量列表
        1. 列表支持正向索引、反向索引、切片索引（`[start:end:stride]`若start值等于0，则可以省略；若end值等于列表长度，则可以省略；若stride值等于1，则可以省略。支持批量修改切片）
        2. 两个列表可以做关系运算（`<=`、`<`、`>`、`>=`、`==`、`!=`），逐个按顺序对比每个项
        3. 运算符支持：`列表 + 列表`、`列表 * 整数`
        4. `in`、`not in`
        6. 生成式创建法
...
1. 集合（set）
    ...
    1. 创建
        ...
        >
        >```py
        >set1 = {1, 2, 3, 3, 3, 2}  // 至少一个元素，否则 空字典：{}
        >print(set1)
        >
        ...
        >```
        ></details>
    1. 集合中的元素必须是`hashable类型`，所谓hashable类型指的是能够计算出哈希码的数据类型，通常不可变类型都是hashable类型，如整数（int）、浮点小数（float）、布尔值（bool）、字符串（str）、元组（tuple）等。可变类型都不是hashable类型，因为可变类型无法计算出确定的哈希码，所以它们不能放到集合中。例如：我们不能将列表作为集合中的元素；同理，由于 集合 本身也是**可变类型**，所以集合也不能作为集合中的元素。我们可以创建出嵌套列表（列表的元素也是列表），但是我们不能创建出嵌套的集合。
    3. `in`、`not in`
