...
这里的 Agentic Engineering 指把软件交付改造成可被智能体理解、执行、验证和沉淀的系统。工程师的重心从逐行实现，转向定义目标、边界、上下文、工具、验证标准和反馈闭环。

这不是放弃开发者身份，而是升级责任边界：人负责产品判断、架构约束、风险识别、验收标准和关键代码审查；AI 在明确约束内完成检索、计划、实现、测试、修复和文档更新。
...
要更多扮演**产品经理、架构师**角色，搭好架构、理清系统，让 AI 在稳定边界内实现具体代码。

>Agentic Engineering：从只写代码，转向设计和管理系统。重点是定义做什么、输出什么、如何验证，再管理执行过程。

**跳出单纯开发者身份**，不要把精力放在逐行控制实现上。代码审查仍然需要，尤其是核心业务逻辑、权限、安全、数据迁移、计费、并发和异常恢复；复杂任务应先审 plan，再审 diff 和测试结果。

还需要扮演**测试**角色，对 AI 产出进行验证。

在让 AI 写代码前，要先把需求聊清楚。可以是“参考某某页面实现导出功能”这样的直接需求，也可以经过多轮澄清，把实现边界推敲清楚后再动手。
...
- 核心原则
    ...
    1. **人审关键决策**：AI 可以先做 review，但核心业务逻辑、权限、安全、数据迁移、计费、并发和异常恢复仍需要人审。
    1. **先强后省**：需求澄清、架构设计、疑难排查用强模型；格式化、批量修改、常规测试和文档整理可用低成本模型。
    1. **工具比提示词更重要**：稳定的命令、脚本、MCP、测试夹具、日志和可观测性，通常比继续打磨 prompt 更能提升产出稳定性。

>学习路径:
>
>1. 精通一个主力工具，例如 Codex、Claude Code 或 Cursor，理解它的上下文、权限、执行和验证机制。
...
>注意：调研类任务需要可联网检索或明确资料来源；工具能力变化很快，选型应看当前项目中的上下文管理、权限边界、执行能力和验证闭环。
