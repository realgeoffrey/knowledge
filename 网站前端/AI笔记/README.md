...
1. 项目初始化
    ...
    1. 创建 `AGENTS.md` 或等价入口文件，记录项目约定、常用命令、测试策略、危险操作边界、改动后验证要求，以及需求迭代后的文档更新规则。
    1. 保持入口文件短而稳定：它只做目录和索引，不写成百科全书。
    1. 将架构图、数据模型、领域术语、外部依赖、部署方式和常见故障处理沉淀为专题文档，由入口文件指向。
    1. 按层级管理上下文：

        1. 全局规则放在入口文件中。
        1. 详细知识放到专题文档中。
        1. 可选：模块业务逻辑放在模块同级目录下，需求文档随实现迭代沉淀到对应代码目录。

        >任务执行时按需加载局部领域知识，避免把所有信息一次性塞入全局上下文。
2. 需求阶段

    >人负责决策“需求澄清与任务分级”：是否需要先进行**人**的 Plan（，抑或都让AI Plan所有），**人**的 Plan 要做到多深，或者需求是否简单到根本不需要人或 AI 单独做 Plan。“Explore first, then plan, then code”

    1. （架构师、产品经理）由**人**进行：产品 Plan / 业务 Plan / 架构 Plan

        >当前因本人工作方式、对流程的把控偏好或工具原因（没有买 cc），这一步会由人完成；也许将来的某一天也可以交给 AI。
    ...
    2. AI 进行：代码库实现 Plan（优先用**强模型**）
        ...
        1. 让 AI 主动提问，补齐缺失信息。
            ...
            todo：<https://github.com/mattpocock/skills/blob/main/skills/productivity/grill-me/SKILL.md>
...
4. <details>
    ...
    1. Review
        ...
        1. 人 review：重点看需求是否被正确实现、架构是否被破坏、关键逻辑是否安全（、代码变动位置是否符合自己预期）。
        - 对复杂任务，同时 review plan、diff、测试结果和文档更新。
    </details>
5. 知识闭环
