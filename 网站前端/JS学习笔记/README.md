...
1. 声明
    ...
    1. 变量声明
        ...
        无论语句在何处，无论是否会真正执行到，所有的`var`语句的**声明**都提升到作用域（函数内部或全局）顶部执行（hoisting），但是具体**赋值**不会被提前。
...
4. `on`绑定效率（事件代理、事件委托）
    ...
    >事件代理：把原本需要绑定的事件委托给祖先元素，让祖先元素担当事件监听的职务。原理是DOM元素的事件冒泡。使用好处是可以提高性能。
...
1. 继承原理
    ...
    >所有函数的原型的最终原型链（`__proto__`）指向Object的原型。因此，所有自定义类型都会继承Object原型上的默认方法：
