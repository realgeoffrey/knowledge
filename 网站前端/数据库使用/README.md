...
- <details>
    ...
    标识符（库 / 表 / 列 / 别名 / 索引名等）和字符串字面量（值）不是一类东西，引号也不同。

    | 东西 | 默认写法 | 例子 |
    | --- | --- | --- |
    | 标识符 | 反引号 `` ` `` | `` SELECT `order-id` FROM `user-db`.`t-orders` `` |
    | 字符串值 | 单引号 `'` | `WHERE name = 'Alice'` |
    | 数字 / 布尔 / `NULL` | 不加引号 | `WHERE id = 1` |

    含 `-`、空格、以数字开头，或与保留字撞名（如 `order`、`select`、`key`）时，标识符**必须**用反引号；用单引号会变成字符串常量，不是对象名：

    ```sql
    SELECT 'order-id' FROM t;   -- ❌：选出常量 'order-id'
    SELECT `order-id` FROM t;   -- ✅：选出列
    ```

    命名优先 `snake_case`、字母数字下划线、避开保留字，多数地方可裸写。已有带 `-` 的名字，凡出现处都加 `` ` ``。限定名按段各自加：`` `库`.`表`.`列` ``。名字里真有反引号时写成两个： ``` `a``b` ```。

    **双引号 `"` 不要当默认习惯用**，含义随 `sql_mode` 变：

    | 模式 | `"` 的含义 | 字符串该用 |
    | --- | --- | --- |
    | 默认（无 `ANSI_QUOTES`） | 表示字符串（非标准扩展） | 仍应写 `'...'`，别依赖 `"` |
    | 开启 `ANSI_QUOTES` | 与反引号同类，表示**标识符** | 只能 `'...'`；`"order-id"` 是列名不是值 |

    ```sql
    -- 默认可移植写法：字符串只用单引号（不要依赖 "）
    SELECT "hello", 'hello';

    SET SESSION sql_mode = CONCAT(@@sql_mode, ',ANSI_QUOTES');
    SELECT "order-id" FROM t;   -- 此时 "order-id" 是标识符，等同 `order-id`
    SELECT 'order-id' FROM t;   -- 仍是字符串常量
    ```

    串内单引号写成 `''`（如 `'it''s'`）。值用参数绑定 / MyBatis `#{}`，不要靠拼引号防注入；`${}` 只用于白名单后的标识符，并再包反引号。
...
- <details>
    ...
    >除 shell 备份 / 重定向导入外均在 `mysql>` 执行（交互内导入见下方 `SOURCE`）。`「...」` 换成实际名。
    ...
    -- 事务
    START TRANSACTION;  -- 等价于： BEGIN; 或 BEGIN WORK;
    ...
    ```shell
    # 导出（逻辑备份；--single-transaction 适合 InnoDB，导出期间尽量少锁）
    mysqldump --single-transaction -h 「主机」 -u 「用户」 -p 「库」 > 「备份.sql」
    ```

    ```shell
    # 执行 / 导入 .sql：不要把多行 SQL 直接粘贴进 mysql>。
    # 粘贴易因终端编码、行尾、半截粘贴出错；含中文注释（-- / /* */）时更容易乱码或把注释截断进语句。
    # 提示符变成 ->、'> 等：还在等语句结束或引号闭合，不一定是报错。
    # 出现 ERROR ...：才是执行失败；前面已成功的语句不会自动撤销。
    # 最佳：在 shell 用重定向整文件执行（默认遇错停止）；目标库一般先建好（除非文件里已有 CREATE DATABASE / USE）
    mysql --default-character-set=utf8mb4 -h 「主机」 -u 「用户」 -p 「库」 < 「脚本.sql」

    # 已在 mysql> 时次选（客户端本地文件；路径相对启动客户端时的 cwd）：
    #   SOURCE 「/绝对或相对路径/脚本.sql」;
    #   \. 「路径」          -- 与 SOURCE 效果相同
    ```
