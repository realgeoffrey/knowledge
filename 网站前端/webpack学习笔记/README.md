...
1. 开发优化体验
    ...
    1. 多进程多实例：
        ...
        2. 压缩：
            ...
            1. ~~[webpack-parallel-uglify-plugin](https://github.com/gdborton/webpack-parallel-uglify-plugin)~~（旧方案）
    ...
    3. 构建缓存
        ...
        3. plugin缓存
            ...
            ~~[hard-source-webpack-plugin](https://github.com/mzgoddard/hard-source-webpack-plugin)~~（webpack5 已内置 filesystem cache，优先使用 webpack 自带缓存）
    4. 减少文件搜索范围
    ...
    5. 开启自动刷新
        ...
        3. （webpack5/webpack-dev-server 新版本删除）`devServer.inline`已不再使用，默认基于 websocket 等机制更新
    6. 开启热更新
...
2. 产物优化（自动）
    ...
    4. [tree shaking](https://github.com/realgeoffrey/knowledge/blob/master/网站前端/webpack学习笔记/README.md#tree-shaking)
        ...
        1. 无用JS删除（主要依赖可静态分析的 ES Module；CommonJS 因动态特性支持有限，不能按 ESM 的效果预期）
        2. 无用CSS删除：
    ...
    6. 动态polyfill
        ...
        >其他polyfill的缺点：
        >
        >1. ~~babel-polyfill~~：已废弃；Babel 7.4+ 推荐直接引入 `core-js/stable` 等所需 polyfill
        >2. `@babel/plugin-transform-runtime`：主要复用 helper、避免污染全局；若要补齐目标环境缺失的内置对象/原型方法，需要结合 `core-js`、`@babel/preset-env` 的 `useBuiltIns` 等策略
        >3. 自己写：重复造轮子，维护问题
...
1. 所有文件都当作是**模块（module）**：`脚本（js、jsx、tsx、coffee）`、`样式（css、scss、sass、less）`、`模版（html、tpl）`、`JSON`、`图片`、`字体`。
    ...
    - 能使用各种方式表达依赖关系
        ...
        CommonJS、ES6 Module、AMD、CSS的`@import`、样式的`url()`、HTML的`<img src="">`。都会被解析进 webpack 依赖图，再由运行时代码按目标环境输出；不是简单全部转成 CommonJS。
...
2. 目标代码
    ...
    >虽然 Webpack 自 2.x 开始就原生支持 Tree Shaking 功能，但受限于 JS 的动态特性与模块副作用，webpack 5 仍需要使用者有意识地优化代码结构、标记 `sideEffects`，或配合压缩工具更精确地删除无效代码。
...
2. （webpack5/webpack-dev-server 新版本已删除）旧版可把要开发的网页装进一个`<iframe>`中，通过刷新`<iframe>`去看到最新效果。
...
App级别的复杂应用（开发、打包）——webpack/Rspack 等。JS库级别的应用（打包）——Rollup。现代浏览器快速开发体验（开发）——Vite。

1. Rollup：

    1. 打包结果文件很小，几乎没多余代码（Tree Shaking）；执行很快；可方便输出CommonJS、ES6 Module、IIFE（用于`<script>`引用）等格式；支持基于多入口和动态 `import()` 的代码分离。
    ...
    - 缺点：面向完整 Web App 的工程能力弱于 webpack/Vite 这类上层工具；CommonJS支持依赖插件且无法做到完全等价；开发服务器、HMR、资源处理等通常交给 Vite 等工具封装。
2. webpack
...
3. vite（`/vit/`）
    ...
    1. 存在开发和构建之间的不一致性
        ...
        1. 开发环境中，利用现代浏览器已支持 ES Module 的 import 能力，遇到 import 就会发送 HTTP 请求去加载 ES Module 文件，整个过程中没有对文件进行打包编译。在开发环境使用 esbuild（Go）进行预构建（提高页面加载速度、将 CommonJS/UMD 转换为 ESM 格式）
        ...
        2. 生产环境中，使用 Rollup/Rolldown 体系打包（具体取决于 Vite 版本）

            >1. 因为每个模块引用都需要 ES Module 网络请求，网络请求开销大，所以生产环境仍使用打包方式。
            >2. Vite 需要兼顾插件生态、产物优化和浏览器兼容性，不能只用 esbuild 的速度作为唯一目标。
    2. 适用于中小型项目，特别是基于现代浏览器的单页应用（SPA）或中小型网站开发。

    - 缺点：开发模式依赖原生 ESM 请求，依赖数量很大或网络条件差时仍可能有请求开销；开发和构建之间存在一定差异，需要关注只在生产构建暴露的问题。

>4. [esbuild](https://github.com/evanw/esbuild)
>
>    **Go**编写的**快速**JS、TS打包器，支持CommonJS（是rollup的很好替代品）。
>5. [Parcel](https://github.com/parcel-bundler/parcel)
>
>    **零配置**的打包工具，适用于小型项目、静态网页或需要快速上手的场景。
>6. [Bun](https://github.com/oven-sh/bun)
>
>    是像 Node.js、Deno 一样的现代 **JS 运行时**，也内置打包器、转译器、任务运行器和 npm 客户端，可作为部分 Node.js 场景的替代选项。兼容性、生态和线上基线仍需按项目验证。Zig 编写。
